...
import { Zap, ShoppingBag, Plus, Minus, CheckCircle, AlertTriangle, ArrowRight } from 'lucide-react';
...
export const SimulatorView: React.FC<SimulatorViewProps> = ({ menuItems, onReceiptProcessed }) => {
  const [cart, setCart] = useState<{ [posItemId: string]: { dish: any; quantity: number } }>({
    'NEX-DISH-001': { dish: menuItems.find((m) => m.posItemId === 'NEX-DISH-001') || { name: 'Бургер Говяжий Классический', sellingPrice: 3500 }, quantity: 2 },
    'NEX-DISH-002': { dish: menuItems.find((m) => m.posItemId === 'NEX-DISH-002') || { name: 'Лимонад Классический 0.5L', sellingPrice: 1500 }, quantity: 1 },
  });
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Тестирование интеграции Event Bus и автосписания сырья в реальном времени</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
        ...
        <div className="card">
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {menuItems.map((dish) => {
                ...
                return (
                  <div key={dish.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: '0.9rem' }}>{dish.name}</div>
                      <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{dish.sellingPrice?.toLocaleString('ru-RU')} ₸</div>
                    </div>
          ...
          <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '16px', marginTop: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>Итого по чеку:</span>
              <span style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--primary)' }}>{totalAmount.toLocaleString('ru-RU')} ₸</span>
            </div>
        ...
        <div className="card" style={{ background: '#090d14', borderColor: 'rgba(16, 185, 129, 0.2)' }}>
          <h2 style={{ fontSize: '1.1rem', fontWeight: 800, marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <ArrowRight size={18} color="var(--primary)" />
            Результат автосписания в Restaurant OS
          </h2>

          {!lastResult ? (
            <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--text-muted)' }}>
              <Zap size={36} color="var(--text-dim)" style={{ marginBottom: '12px' }} />
              <p>Нажмите <strong>«Провести чек»</strong>, чтобы увидеть, как Restaurant OS обрабатывает списание сырья по рецептуре.</p>
            </div>
          ) : (
            <div>
              <div style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.3)', padding: '14px', borderRadius: '10px', marginBottom: '16px' }}>
              ...
              <h4 style={{ fontSize: '0.9rem', fontWeight: 700, marginBottom: '10px' }}>Списанное сырье со склада:</h4>
              ...
              {lastResult.incidents && lastResult.incidents.length > 0 && (
                <div style={{ background: 'rgba(244, 63, 94, 0.1)', border: '1px solid rgba(244, 63, 94, 0.3)', padding: '12px', borderRadius: '8px' }}>
                  <div style={{ color: '#f87171', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.85rem', marginBottom: '6px' }}>
                    <AlertTriangle size={16} /> Обнаружен дефицит сырья:
                  </div>
